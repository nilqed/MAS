...
(*2*) (*compute zeta, eta, theta, delta, epsil and tabp2 elements.*)
      IL:=1; TL:=1;
      WHILE TL < BETA DO TABP2[VAL(INTEGER,IL)]:=TL; 
      ...
      J1Y:=ZETA DIV 2; J1Y:=J1Y+1; DELTA:=TABP2[VAL(INTEGER,J1Y)]; 
...
(*2*) (*binary search.*) IL:=1; JL:=ZETA+1;
      REPEAT J1Y:=IL+JL; NL:=J1Y DIV 2;
             IF ALB >= TABP2[VAL(INTEGER,NL)] THEN IL:=NL; ELSE JL:=NL; END;
             UNTIL JL-IL = 1;
      NL:=IL; RETURN(NL);
(*5*) END DLOG2;
...
(*3*) (*shift and subtract.*) QLP:=0;
      FOR IL:=1 TO VAL(INTEGER,ZETA) DO ALP1:=ALP1+ALP1; ALP0:=ALP0+ALP0;
...
(*2*) (*compute first approximation.*) KL:=DLOG2(AL); J1Y:=KL+1;
      HL:=J1Y DIV 2; BL:=TABP2[VAL(INTEGER,HL)+1];
