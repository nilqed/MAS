...
(*2*) (*general case.*) QL:=KL DIV ZETA; J1Y:=QL*ZETA; RL:=KL-J1Y; B:=A;
      ...
      B:=IDQ(B,TABP2[VAL(INTEGER,RL+1)]); RETURN(B);
(*5*) END IDP2;
...
(*2*) (*general case.*) QL:=HL DIV ZETA; J1Y:=QL*ZETA; RL:=HL-J1Y;
      B:=IDPR(A,TABP2[VAL(INTEGER,RL+1)]);
      IF QL <> 0 THEN
         IF B < BETA THEN B:=LIST1(B); END;
...
(*1*) (*compute sign and high-order beta-digit.*) MASQREM(NL,ZETA,QL,RL);
      ...
      AL:=AL DIV TABP2[VAL(INTEGER,J1Y)];
...
(*2*) (*let abs(nl)=ql*zeta+rl, al=2**rl.*) ML:=MASABS(NL); QL:=ML DIV ZETA;
      J1Y:=QL*ZETA; RL:=ML-J1Y; AL:=TABP2[VAL(INTEGER,RL+1)];
(*3*) (*nl gt 0.*)
      IF NL > 0 THEN IL:=0;
         IF B < BETA THEN B:=LIST1(B); END;
...
(*4*) (*write remaining digits.*)
      WHILE H <> SIL DO ADV(H,HL,H); HL:=MASABS(HL);
            FOR IL:=1 TO VAL(INTEGER,ETA) DO QL:=HL DIV 10; J1Y:=10*QL; 
                B[IL]:=HL-J1Y;
                HL:=QL; END;
            FOR IL:=VAL(INTEGER,ETA) TO 1 BY -1 DO CWRITE(B[IL]); END;
            END;
      RETURN;
(*7*) END IWRITE;
