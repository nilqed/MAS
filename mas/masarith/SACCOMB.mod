...
BEGIN
(*1*) (*generate random permutation.*) n:=VAL(INTEGER,NL); 
      FOR IL:=1 TO n DO A[IL]:=IL; END; 
      FOR JL:=n TO 2 BY -1 DO J1Y:=VAL(INTEGER,MDRAN(GAMMAINT(VAL(LIST,JL)))); 
          IL:=J1Y+1; KL:=A[IL]; 
...
BEGIN
(*1*) (*initialize array.*) n:=VAL(INTEGER,NL); 
      FOR IL:=1 TO n DO A[IL]:=IL; END; 
(*2*) (*random interchanges.*) 
      FOR JL:=n TO 1 BY -1 DO J1Y:=VAL(INTEGER,MDRAN(GAMMAINT(VAL(LIST,JL)))); 
          IL:=J1Y+1; TL:=A[IL]; 
          A[IL]:=A[JL]; A[JL]:=TL; END; 
(*3*) (*form list.*) L:=BETA; 
      FOR IL:=1 TO n DO L:=COMP(GAMMAINT(VAL(LIST,A[IL])),L); END; 
      RETURN(L); 
(*6*) END PERMR; 
