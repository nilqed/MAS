      ...
      LOOP   QL:=ML2 DIV DL;
             ...
             IF (RL+HL >= DL) OR (RL = 0) THEN
                IF RL = 0 THEN i:=1; ELSE
                   IF RL MOD 2 = 0  
                      THEN i:=VAL(INTEGER,DL-(RL DIV 2)+1); 
                      ELSE i:=VAL(INTEGER,((DL-RL) DIV 2)+1); END;
                   END;
                IF ML1 <= DL THEN i:=i+VAL(INTEGER,DL); END;
                WHILE i <= k DO P[i]:=FALSE; i:=i+VAL(INTEGER,DL); END;
                END;
...
(*2*) (*mark proper multiples of dl=3, dl=6*nl+1 and dl=6*nl-1 with
      ...
      LOOP   QLB:=ML2 DIV DL;
             ...
             i:=VAL(INTEGER,J1Y)+1; QL:=QLB-JL;
             WHILE (i > 0) AND (QL > 1) DO P[i]:=FALSE; i:=i-VAL(INTEGER,DL);
                   QL:=QL-4; END;
